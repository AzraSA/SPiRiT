function [COVID_Percetage,Pf_DP_ACT,Pf_AP_DP_3T,Pf_SPiRiT,Pf_DP_3T, high_risk_percentage_AP_DP3T, high_risk_percentage_DP3T, high_risk_percentage_DPACT]=High_risk_case_detection_versus_COVID_Percentage_FP(A2,K,B,  Bfa)
...
% Bfa is Initial False Alarm

% Outputs:
% Pf_DP_ACT is false alarm rate for DP_ACT
% Pf_AP_DP_3T is false alarm rate for A/P_DP_3T
% Pf_SPiRiT is false alarm rate for SPiRiT
% Pf_DP_3T is false alarm rate for DP-3T
...
Bmax=50;
high_risk_percentage_SPiRiT=zeros(1,Bmax/10);
high_risk_percentage_DP_3T=zeros(1,Bmax/10);high_risk_percentage_AP_DP_3T=zeros(1,Bmax/10);
high_risk_percentage_DP_ACT=zeros(1,Bmax/10);
high_risk_percentage5_tf=zeros(1,Bmax/10);Pf_DP_ACT=zeros(1,Bmax/10);Pf_AP_DP_3T=zeros(1,Bmax/10);Pf_SPiRiT=zeros(1,Bmax/10);
Pf_DP_3T=zeros(1,Bmax/10);
...
for  C=10:10:Bmax
    ...
    for kk=1:K
        ...
        ss2=ceil(abs(rand(1,ceil(C/100*l2)))*days);
        ...
        for K1=1:1:ceil(BB)
            ...
            [covid_indexes2]=high_risk_only_first_contact_revision(X_duration_advertize_after_delay,duration_threshold,covid_indexes2,l22,l2,erased_indexes,unerased_indexes);
            vv1=[vv1; covid_indexes2'];

            [covid_indexes3]=DP_3T(X_duration_advertize_after_delay,duration_threshold,covid_indexes3,l22,l2,erased_indexes,unerased_indexes);
            vv2=[vv2; covid_indexes3'];

            [covid_indexes4]=DP_ACT(X_duration_after_delay,X_duration_advertize_after_delay,duration_threshold,covid_indexes4,l22,l2,erased_indexes,unerased_indexes);
        ...
        xx_l=ceil((C+Bfa)/100*l2);
        covid_indexesff=zeros(1,xx_l);
        covid_indexesff(1,1:size(covid_indexes,2))=covid_indexes;
        %kk=1;
        ...
        set1 = setdiff(1:l2, covid_indexes1);
        aa=randperm(length(set1));index_aa=aa(1:xx_l-size(covid_indexes,2));
        covid_indexesff(size(covid_indexes,2)+1:xx_l)=set1(index_aa);   %it is random

        covid_indexes_array1f={covid_indexesff;covid_indexesff;covid_indexesff;covid_indexesff};
        covid_indexes_array2f={covid_indexesff;covid_indexesff;covid_indexesff;covid_indexesff};
        covid_indexes_array3f={covid_indexesff;covid_indexesff;covid_indexesff;covid_indexesff};
        covid_indexes_array4f={covid_indexesff;covid_indexesff;covid_indexesff;covid_indexesff};
        for K1=1:1:ceil(BB)
            covid_indexes1f=covid_indexes_array1f{rem(K1,4)+1};
            covid_indexes2f=covid_indexes_array2f{rem(K1,4)+1};
            covid_indexes3f=covid_indexes_array3f{rem(K1,4)+1};
            covid_indexes4f=covid_indexes_array4f{rem(K1,4)+1};
            X_duration_after_delay=cell(l2,1);
            X_duration_advertize_after_delay=cell(l2,1);
            for i=1:l2
                for j=1:size(X_duration{i},2)
            ...
            [covid_indexes1f]=high_risk1(X_duration_after_delay,duration_threshold,covid_indexes1f,l22,l2);
            vvf=[vvf; covid_indexes1f'];

            [covid_indexes2f]=high_risk_only_first_contact_revision(X_duration_advertize_after_delay,duration_threshold,covid_indexes2f,l22,l2,erased_indexes,unerased_indexes);
            vv1f=[vv1f; covid_indexes2f'];

            [covid_indexes3f]=DP_3T(X_duration_advertize_after_delay,duration_threshold,covid_indexes3f,l22,l2,erased_indexes,unerased_indexes);
            vv2f=[vv2f; covid_indexes3f'];

            [covid_indexes4f]=DP_ACT(X_duration_after_delay,X_duration_advertize_after_delay,duration_threshold,covid_indexes4f,l22,l2,erased_indexes,unerased_indexes);
            vv3f=[vv3f; covid_indexes4f'];
            covid_indexes_array1{rem(K1,4)+1}=covid_indexes1f;
            covid_indexes_array2{rem(K1,4)+1}=covid_indexes2f;
            covid_indexes_array3{rem(K1,4)+1}=covid_indexes3f;
            covid_indexes_array4{rem(K1,4)+1}=covid_indexes4f;
        end
        v2_ref=[];
        for i=1:l2
            if ismember(i,vvf)
        ...
        high_risk_ref1=high_risk_ref1+numel(v2_ref);
        for i=1:l2
            if sum(i==vv1f)
                if ismember(i,vv)
                    if (i~=covid_indexes)
                        high_risk_percentage3f=high_risk_percentage3f+1;
                    end
                else
                    if (i~=covid_indexes1)
                        pfaf_ap=pfaf_ap+1;
        ...
        for i=1:l2
            if sum(i==vv2f)
                if ismember(i,vv)
                    if (i~=covid_indexes)
                        high_risk_percentage4f=high_risk_percentage4f+1;
                    end
                else
                    if (i~=covid_indexes1)
                        pfaf_dp3t=pfaf_dp3t+1;
                    end
                end
            end
        end
        for i=1:l2
            if ismember(i,vv3f)
                if ismember(i,vv)
                    if (i~=covid_indexes)
                        high_risk_percentage5f=high_risk_percentage5f+1;
                    end
                else
                    if (i~=covid_indexes1)
                        pfaf=pfaf+1;
    ...
    high_risk_percentage_SPiRiT(C/10)=high_risk_percentage3/high_risk_ref;
    high_risk_percentage_AP_DP3T(C/10)=high_risk_percentage3f/high_risk_ref; 
    high_risk_percentage_DP3T(C/10)=high_risk_percentage4f/high_risk_ref;
    high_risk_percentage_DP_ACT(C/10)=high_risk_percentage5f/high_risk_ref;
    Pf_SPiRiT(C/10)=pfa_ap/l2/K*100;
    Pf_DP_ACT(C/10)=pfaf/l2/K*100;
    Pf_AP_DP_3T(C/10)=pfaf_ap/l2/K*100;
    Pf_DP_3T(C/10)= pfaf_dp3t/l2/K*100;
end
end